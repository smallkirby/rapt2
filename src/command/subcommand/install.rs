...
use crate::{
  ...
  net::binary::BinaryDownloader, package::client::PackageClient, source::client::SourceClient,
};
...
pub fn execute(context: &Context, args: &InstallArgs) -> Result<(), RaptError> {
  ...
  let binary_client = BinaryDownloader::new(sorted_deps, PathBuf::from(&context.archive_dir))?;
